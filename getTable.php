...
echo "<thead>
	...
	<tbody>";
$query = "SELECT `product`.`id`,`product`.`name`,`product`.`price`, `category`.`name` as `cateogry` FROM `product` LEFT JOIN `category` ON `product`.`category` = `category`.`id` WHERE `category`.`storage`='{$group}'";
